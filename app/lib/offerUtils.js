...
export const calculateOffers = (cartItems, allProducts) => {
  const freeItems = [];

  // Check for Coca-Cola offer
  const cokeItems = cartItems.filter(
    (item) =>
      item.productId.toString() === OFFERS.COCA_COLA.requiredProductId && !item.isOffer
  );
  const totalCokeQuantity = cokeItems.reduce(
    (sum, item) => sum + item.quantity,
    0
  );

  const freeCokeCount = Math.floor(
    totalCokeQuantity / OFFERS.COCA_COLA.requiredQuantity
  );
  ...
  // Check for Croissant + Coffee offer
  const croissantItems = cartItems.filter(
    (item) =>
      item.productId.toString() === OFFERS.CROISSANT_COFFEE.requiredProductId &&
      !item.isOffer
  );
  const totalCroissantQuantity = croissantItems.reduce(
    (sum, item) => sum + item.quantity,
    0
  );
  ...
  if (freeCoffeeCount > 0) {
    ...
    // Only add free coffee if none was manually added and it's not already offered
    if (coffeeProduct ) {
      freeItems.push({
...
export const shouldRemoveFreeItem = (cartItems, freeItem) => {
